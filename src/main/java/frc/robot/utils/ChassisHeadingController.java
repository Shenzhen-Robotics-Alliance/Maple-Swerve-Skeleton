...
public class ChassisHeadingController {
     ...
     */
    public static abstract class ChassisHeadingRequest {}

    /**
     * <h2>Represents a request to face a specific rotation.</h2>
     * <p>The chassis is instructed to rotate to and maintain a specified rotation.</p>
     */
     ...
     *
     * <p>The chassis is instructed to aim toward a specified target on the field.
     * Optionally, a {@link MapleShooterOptimization} object can be provided to calculate
     * time-of-flight for shooting-on-the-move functionality.</p>
     */
    public static class FaceToTargetRequest extends ChassisHeadingRequest {
         ...
         * @param shooterOptimization optional; used to calculate shooter time-of-flight
         *                            for shooting-on-the-move functionality; if not used, pass null
         */
     ...
     *
     * <p>This request will cause {@link #calculate(ChassisSpeeds, Pose2d)} to return
     * zero correction speeds.</p>
     */
     ...
     */
    public ChassisHeadingController(TrapezoidProfile.Constraints chassisRotationConstraints, MaplePIDController.MaplePIDConfig chassisRotationCloseLoopConfig, Rotation2d chassisInitialFacing) {
    ...
    public OptionalDouble calculate(ChassisSpeeds measuredSpeedsFieldRelative, Pose2d robotPose) {
        if (headingRequest instanceof FaceToRotationRequest faceToRotationRequest) {
            return OptionalDouble.of(calculateFaceToRotation(
                    robotPose,
                    faceToRotationRequest.rotationTarget,
                    0
            ));
        }
     ...
     *
     * <p>For a continuously changing target, feed-forward velocity is added to help the
     * chassis stay aligned. This feed-forward is based on the target's angular velocity relative
     * to the robot, improving tracking accuracy.</p>
     *
     * @param measuredSpeedsFieldRelative the measured chassis speeds, field-relative
     * @param robotPose the current pose of the robot as measured by odometry
     ...
     */
    private double calculateFaceToTarget(ChassisSpeeds measuredSpeedsFieldRelative, Pose2d robotPose, Translation2d targetPosition, MapleShooterOptimization shooterOptimization) {
        // Target velocity relative to the robot, in the field-origin frame
        final Translation2d targetMovingSpeed = new Translation2d(-measuredSpeedsFieldRelative.vxMetersPerSecond, -measuredSpeedsFieldRelative.vyMetersPerSecond);

        final Rotation2d
                targetedRotation = shooterOptimization == null ?
                targetPosition.minus(robotPose.getTranslation()).getAngle()
                : shooterOptimization.getShooterFacing(targetPosition, robotPose.getTranslation(), measuredSpeedsFieldRelative),

                targetMovingDirection = new Translation2d(-measuredSpeedsFieldRelative.vxMetersPerSecond, -measuredSpeedsFieldRelative.vyMetersPerSecond)
                        .getAngle(),

                positiveRotationTangentDirection = targetPosition.minus(robotPose.getTranslation())
                        .getAngle()
                        .rotateBy(Rotation2d.fromDegrees(90));

        final double tangentVelocity = targetMovingDirection
                .minus(positiveRotationTangentDirection)
                .getCos() * targetMovingSpeed.getNorm(),
                angularVelocity = tangentVelocity / targetPosition.minus(robotPose.getTranslation()).getNorm();
     ...
     */
    private double calculateFaceToRotation(Pose2d robotPose, Rotation2d targetedRotation, double desiredAngularVelocityRadPerSec) {
        TrapezoidProfile.State goalState = getGoalState(targetedRotation);
        chassisRotationState = chassisRotationProfile.calculate(Robot.defaultPeriodSecs, chassisRotationState, goalState);

        final double feedBackSpeed = chassisRotationCloseLoop.calculate(robotPose.getRotation().getRadians(), chassisRotationState.position);
        final double feedForwardSpeedRadPerSec = Math.abs(targetedRotation.minus(robotPose.getRotation()).getDegrees()) < 15 ?
                desiredAngularVelocityRadPerSec : chassisRotationState.velocity;

        log(robotPose, targetedRotation);

        return MapleCommonMath.constrainMagnitude(feedBackSpeed + feedForwardSpeedRadPerSec, maxAngularVelocityRadPerSec);
    }
     ...
     * This ensures continuity in the rotational profile.</p>
     *
    ...
    private TrapezoidProfile.State getGoalState(Rotation2d targetedRotation) {
        final Rotation2d difference = targetedRotation
                .minus(Rotation2d.fromRadians(chassisRotationState.position));
