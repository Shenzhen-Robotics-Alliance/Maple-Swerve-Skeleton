...
public class ModuleIOSim implements ModuleIO {
    ...
    public void updateInputs(ModuleIOInputs inputs) {
        ...
        inputs.driveWheelFinalVelocityRevolutionsPerSec =
                Units.radiansToRotations(physicsSimulationResults.driveWheelFinalVelocityRadPerSec);
        ...
        inputs.odometryDriveWheelRevolutions =
                Arrays.copyOf(physicsSimulationResults.odometryDriveWheelRevolutions, SIMULATION_TICKS_IN_1_PERIOD);
        inputs.odometrySteerPositions =
                Arrays.copyOf(physicsSimulationResults.odometrySteerPositions, SIMULATION_TICKS_IN_1_PERIOD);
    ...
    public double getSimulationTorque() {
        ...
        final double currentAtDesiredVolts = DRIVE_MOTOR.getCurrent(driveMotorRotterSpeedRadPerSec, driveAppliedVolts);
        ...
        if (Math.abs(currentAtDesiredVolts) > 1.5 * DRIVE_CURRENT_LIMIT
            ...
            driveAppliedVolts =
                    DRIVE_MOTOR.getVoltage(DRIVE_MOTOR.getTorque(limitedCurrent), driveMotorRotterSpeedRadPerSec);
        }

        driveActualCurrent = DRIVE_MOTOR.getCurrent(driveMotorRotterSpeedRadPerSec, driveAppliedVolts);
    ...
    public SwerveModuleState getDesiredSwerveState() {
        return new SwerveModuleState(
                DRIVE_MOTOR.getSpeed(0, driveDesiredVolts) * WHEEL_RADIUS_METERS, getSimulationSteerFacing());
    }
    ...
    public static class SwerveModulePhysicsSimulationResults {
        ...
        public final double[] odometryDriveWheelRevolutions = new double[SIMULATION_TICKS_IN_1_PERIOD];
        public final Rotation2d[] odometrySteerPositions = new Rotation2d[SIMULATION_TICKS_IN_1_PERIOD];
