    ...
    class CameraInputs {
        ...
        public void fromPhotonPipeLine(PhotonPipelineResult pipelineResult, boolean cameraConnected) {
            ...
            this.currentTargetsCount =
                    Math.min(pipelineResult.getTargets().size(), MAX_TARGET_PER_CAMERA);
            Arrays.fill(fiducialMarksID, -1);
            Arrays.fill(bestCameraToTargets, new Transform3d());
            ...
            this.bestCameraToField =
                    pipelineResult.getMultiTagResult().estimatedPose.isPresent
                            ? Optional.of(pipelineResult.getMultiTagResult().estimatedPose.best)
                            : Optional.empty();
        }

        private static final Transform3d NULL_TRANSFORM =
                new Transform3d(-114514, -114514, -114514, new Rotation3d());
        ...
        public void fromLog(LogTable table, int cameraID) {
            ...
            final int[] fiducialMarkIDLogged =
                    table.get(cameraKey + "FiducialMarksID", new int[MAX_TARGET_PER_CAMERA]);
            ...
            Transform3d bestCameraToFieldTransform =
                    table.get(cameraKey + "bestCameraToField", NULL_TRANSFORM);
            this.bestCameraToField =
                    bestCameraToFieldTransform.equals(NULL_TRANSFORM)
                            ? Optional.empty()
