...
public class MapleMultiTagPoseEstimator {
    ...
    private void applyFilteringToRawRobotPose3dEstimations() {
        ...
        for (final Pose3d estimation : robotPose3dObservations) {
            if (filter.isResultValid(estimation)) validRobotPoseEstimations.add(estimation);
    ...
    public Optional<RobotPoseEstimationResult> estimateRobotPose(
        ...
        if (Robot.CURRENT_ROBOT_MODE != RobotMode.REAL) logFilteringData();
    ...
    private void logFilteringData() {
        ...
        Logger.recordOutput(
                APRIL_TAGS_VISION_PATH + "Filtering/ValidPoseEstimations",
