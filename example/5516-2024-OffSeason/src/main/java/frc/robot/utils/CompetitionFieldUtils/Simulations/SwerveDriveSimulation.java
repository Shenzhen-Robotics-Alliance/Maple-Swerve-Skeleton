    ...
    public void updateSimulationSubTick(int tickNum, double tickSeconds) {
        for (int i = 0; i < modules.length; i++)
            moduleSimulationSubTick(
                    getObjectOnFieldPose2d(), modules[i], MODULE_TRANSLATIONS[i], i, tickNum, tickSeconds);
    ...
    private void moduleSimulationSubTick(
        ...
        if (skidding)
            ...
            module.physicsSimulationResults.driveWheelFinalVelocityRadPerSec +=
                    module.getSimulationTorque() / DRIVE_INERTIA * tickPeriodSeconds * 0.3;
        ...
        Logger.recordOutput(
                "MaplePhysicsSimulation/module" + moduleIndex + "propelling force",
                actualPropellingForceOnFloorNewtons);
