...
public class ScoreAmp extends Command {
    ...
    public void execute() {
        ...
        if (startTime > START_PITCH_DELAY) pitch.runSetPointProfiled(Math.toRadians(98));
        else pitch.runSetPointProfiled(60);
    ...
    // Visualization
    public Command withVisualization(
        ...
        final Command visualizeAmpNote = Commands.runOnce(() -> {
            final Transform2d poseError = FieldConstants.toCurrentAlliancePose(CORRECT_SCORE_AMP_ROBOT_POSE_BLUE)
                    .minus(driveSubsystem.getPose());
            final Translation2d shooterPositionOnField = driveSubsystem
                    .getPose()
                    .getTranslation()
            ...
            if (poseError.getTranslation().getNorm() < 0.25
                ...
                visualizer.addGamePieceOnFly(
                        new AmpFailedNote(driveSubsystem.getPose().getRotation(), shooterPositionOnField, simulation));
        });
        final Command visualizationCommand =
                Commands.waitSeconds(0.3).andThen(visualizeAmpNote).onlyIf(() -> hasNote && visualizer != null);
        return this.alongWith(visualizationCommand);
    }

    private static final double SHOOTER_HEIGHT_AMP = 0.4, AMP_INITIAL_VELOCITY = 3.5, AMP_FLIGHT_TIME_SECONDS = 0.8;
    ...
    private static final class AmpSuccessNote extends GamePieceOnFlyDisplay {
        ...
        public Pose3d getPose3d() {
            ...
            final double t = getTimeSinceLaunchSeconds(),
                    height = SHOOTER_HEIGHT_AMP + AMP_INITIAL_VELOCITY * t - 1.0 / 2.0 * t * t * 10;
            return new Pose3d(
                    new Translation3d(
    ...
    private static final class AmpFailedNote extends GamePieceOnFlyDisplay {
        ...
        public AmpFailedNote(
                Rotation2d robotFacingWhenLaunching,
            ...
            super(
                    ...
                    getNoteTouchGroundTranslation3d(
                            shooterPositionOnField.plus(new Translation2d(-0.7, 0).rotateBy(robotFacingWhenLaunching))),
                    AMP_FLIGHT_TIME_SECONDS);
            this.robotFacingWhenLaunching = robotFacingWhenLaunching;
            if (simulation != null)
                CommandScheduler.getInstance()
                        .schedule(Commands.waitSeconds(AMP_FLIGHT_TIME_SECONDS)
