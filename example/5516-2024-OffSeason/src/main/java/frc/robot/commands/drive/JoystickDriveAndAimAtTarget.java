        ...
        final Rotation2d
                targetedFacing =
                        shooterOptimization.getShooterFacing(
                                targetPositionSupplier.get(), robotPosition, robotVelocityFieldRelative),
                /* to calculate the derivative of target facing */
                targetedFacingAfterDT =
                        shooterOptimization.getShooterFacing(
                                targetPositionSupplier.get(), robotPositionAfterDt, robotVelocityFieldRelative);
        final double targetedFacingChangeRateRadPerSec =
                targetedFacingAfterDT.minus(targetedFacing).getRadians() / Robot.defaultPeriodSecs;
        Logger.recordOutput("Drive/Face To Target Rotation (Deg)", targetedFacing.getDegrees());

        final double
                feedBackRotationalSpeed =
                        chassisRotationController.calculate(
                                driveSubsystem.getFacing().getRadians(), targetedFacing.getRadians()),
                feedForwardRotationalSpeed = targetedFacingChangeRateRadPerSec * FEED_FORWARD_RATE;

        final double chassisRotationalError =
                Math.abs(targetedFacing.minus(driveSubsystem.getFacing()).getDegrees());
