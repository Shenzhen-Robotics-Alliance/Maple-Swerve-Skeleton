...
public class FeedShot {
    ...
    public static Command shootFeed(
        ...
        shootFeed.addCommands(prepareToFeedUntilReady(pitch, flyWheels).withTimeout(0.5));

        shootFeed.addCommands(
                intake.executeLaunch().withTimeout(2).deadlineWith(prepareToFeedForever(pitch, flyWheels)));
