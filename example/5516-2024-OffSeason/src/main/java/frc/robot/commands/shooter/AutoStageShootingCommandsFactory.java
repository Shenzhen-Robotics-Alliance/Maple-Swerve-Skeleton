...
public class AutoStageShootingCommandsFactory {
    ...
    public static final Pose2d PRECISE_ALIGNMENT_TOLERANCE =
    ...
    public static Command followPathGrabAndShoot(
        ...
        final Supplier<Translation2d> scoringPositionSupplier =
                () ->
                        getEndingRobotPoseInCurrentAllianceSupplier(pathAtBlueAlliance).get().getTranslation();
        final Command followPathUntilAlmostFinished =
                AutoBuilder.followPath(pathAtBlueAlliance)
                        .until(() -> pathAlmostFinish(pathAtBlueAlliance, driveSubsystem.getPose()));
        final Supplier<Pose2d> preciseAlignmentTarget =
                () ->
                        new Pose2d(
        ...
        final BooleanSupplier robotScoringPoseReached =
                () ->
                        preciseAlignmentFinished(
                                preciseAlignmentTarget.get(),
                                driveSubsystem.getPose(),
                                driveSubsystem.getMeasuredChassisSpeedsRobotRelative());

        final PrepareToAim prepareToAim =
                new PrepareToAim(
                        flyWheels,
                        pitch,
                        shooterOptimization,
        ...
        final Command intakeNoteDuringBeforeAimSuccess =
                grabNoteAtShootingPosition
                        ? intake.executeIntakeNote().onlyIf(() -> !intake.isNotePresent())
                        : Commands.none();
        final Command pushNoteUpwardsAfterAimSuccess =
                grabNoteAtShootingPosition
                        ? Commands.run(intake::runFullIntakeVoltage, intake).until(intake::isNoteTouchingIntake)
                        : Commands.none();
        final Command shootNoteAfterAimSuccess =
                pushNoteUpwardsAfterAimSuccess
                        .andThen(
        ...
        final Command overallIntakeExecution =
                intakeNoteDuringBeforeAimSuccess
                        .andThen(
                                Commands.waitUntil(
                                        (() -> prepareToAim.isReady() && robotScoringPoseReached.getAsBoolean())))
    ...
    private static boolean pathAlmostFinish(PathPlannerPath pathAtBlueAlliance, Pose2d currentPose) {
        ...
        final double distanceToTarget =
                currentPose.getTranslation().getDistance(endingPose.getTranslation());
